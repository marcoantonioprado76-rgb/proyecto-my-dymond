...
import { generateWhatsAppChat } from '@/lib/ads/openai-ads'
...
export async function POST(req: Request, { params }: { params: { id: string } }) {
    ...
    try {
        ...
        const chat = await generateWhatsAppChat({
            brief: {
                name: b.name,
                industry: b.industry,
        ...
        if (resolved.source === 'admin' && usage) {
            ...
            chargeForChatUsage(user.id, model, u.promptTokens, u.completionTokens, 'ads.whatsappChat', { campaignId: params.id })
                .catch(e => console.error('[WhatsAppChat] charge error:', e))
        }

        return NextResponse.json({ chat })
    } catch (e: any) {
        console.error('[POST /api/ads/campaign/[id]/whatsapp-chat]', e)
        return NextResponse.json({ error: e?.message || 'No se pudo generar el chat' }, { status: 500 })
    }
}
