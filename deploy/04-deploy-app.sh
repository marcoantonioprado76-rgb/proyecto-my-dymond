...
for i in $(seq 1 40); do
  if $SSH "test -f /var/log/agentenuro-bootstrap.done" 2>/dev/null; then echo "  bootstrap OK"; break; fi
  sleep 15
done

echo "[deploy] clonando/actualizando repo en /opt/agentenuro ..."
$SSH "bash -s" <<EOF
set -e
if [ ! -d /opt/agentenuro/.git ]; then
  git clone --branch "$GIT_BRANCH" "$GIT_REPO" /opt/agentenuro
else
  cd /opt/agentenuro && git fetch origin "$GIT_BRANCH" && git checkout "$GIT_BRANCH" && git reset --hard origin/"$GIT_BRANCH"
fi
EOF

echo "[deploy] copiando .env de producción..."
scp -i "$KEY_FILE" -o StrictHostKeyChecking=no "$SECRETS_DIR/prod.env" ubuntu@"$EIP":/opt/agentenuro/.env
...
scp -i "$KEY_FILE" -o StrictHostKeyChecking=no "$DEPLOY_DIR/agentenuro.service" ubuntu@"$EIP":/tmp/agentenuro.service
scp -i "$KEY_FILE" -o StrictHostKeyChecking=no "$DEPLOY_DIR/Caddyfile"        ubuntu@"$EIP":/tmp/Caddyfile
$SSH "bash -s" <<'EOF'
set -e
sudo mv /tmp/agentenuro.service /etc/systemd/system/agentenuro.service
sudo mv /tmp/Caddyfile /etc/caddy/Caddyfile
sudo mkdir -p /var/log/caddy
sudo systemctl daemon-reload
sudo systemctl enable agentenuro
sudo systemctl restart agentenuro
sudo systemctl restart caddy
sleep 3
sudo systemctl --no-pager status agentenuro | head -5
EOF
...
echo "  - Logs de la app:  ssh -i $KEY_FILE ubuntu@$EIP 'journalctl -u agentenuro -f'"
